...
func SetupRouter(a *app.App) *gin.Engine {
	...
	api := r.Group("/api")
	{
		v1 := api.Group("/v1")
		{
			...
			protected.GET("/room/:slug/history", a.RoomHandler.GetHistory)
			protected.POST("/room/:slug/leave", a.RoomHandler.LeaveRoom)
			protected.DELETE("/room/:slug", a.RoomHandler.DeleteRoom)
